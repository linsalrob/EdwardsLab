...
def parse_text_file(tf):
    ...
    with open(tf, 'r') as fin:
        for l in fin:
            p=l.strip().split("\t")
            if len(p) != 4:
                sys.stderr.write(f"ERROR: {l} does not have four entries. Expected [item1, item2, pearsonr, pvalue] Skipped\n")
                continue
            ks.add(p[0])
            ks.add(p[1])
            if p[0] not in data:
            ...
            if p[1] not in data:
                data[p[1]] = {}
            data[p[0]][p[1]] = float(p[2])
            data[p[1]][p[0]] = float(p[2])
    ...
    for i in range(len(allkeys)):
        for j in range(i+1, len(allkeys)):
            nct[p] = data[allkeys[i]][allkeys[j]]
            p += 1
    return nct
