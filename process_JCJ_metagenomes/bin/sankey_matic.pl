...
while (<IN>) {
	...
	if (!@headers) {
		...
		$shh = &find_idx(\@headers, "human");
		$nsh = &find_idx(\@headers, "no_human");
		$mmh = &find_idx(\@headers, "mmseqs");
		next;
	}
	map {$counts->[$_] += @a[$_]} (1 .. $#a);
	if ($a[$fph] > $a[$fqh]) {print STDERR "$a[0]: More fastp than fastq reads\n"}
	if ($a[$shh] + $a[$nsh] > $a[$fph]) {print STDERR "$a[0]: More human+no human than fastp reads\n"}
	if ($a[$mmh] > $a[$nsh]) {print STDERR "$a[0]: More mmseqs reads than no humans\n"}
}
...
my $human =  $counts->[&find_idx(\@headers, "human")];
my $nohuman = $counts->[&find_idx(\@headers, "no_human")];
my $lost = $fp - ($human + $nohuman);
my $mmseq = $counts->[&find_idx(\@headers, "mmseqs")];
my $nosim = $nohuman - $mmseq;
...
print "fastp [", $human, "] human\n";
print "fastp [", $nohuman, "] not human\n";
if ($lost > 0) {
	print "fastp [", $lost, "] lost\n";
}
print "not human [", $mmseq, "] sequence similarity\n";
print "not human [", $nosim, "] unknown\n";
