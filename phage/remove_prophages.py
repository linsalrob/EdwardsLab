...
def next_location(s, posn, delimiters):
    """
    Find the next instance of the delimiter in the list of delimiters in the string s
    :param s: a string to search through
    :param posn: current position to start at. Note we start at that posn. 0==start of string
    ...
    return lowest, enddelim, s[posn:lowest]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Generate fasta files without prophages",
                                     epilog='Note that we expect a seed directory in -s for every phispy directory (in -p) and that the seed directory contains a contigs file')
    ...
    for phispydir in os.listdir(args.p):
        if not os.path.exists(os.path.join(args.s, phispydir)):
            if args.v:
                sys.stderr.write("A seed directory matching the phispy directory {} was not found. Skipped\n".format(phispydir))
            continue
        ...
        if not os.path.exists(os.path.join(args.s, phispydir, "contigs")):
            sys.stderr.write("Error: no contigs file was found at {}. Skipped\n".format(os.path.join(args.s, phispydir, "contigs")))
        ...
        for gid, genomecontig in roblib.stream_fasta(os.path.join(args.s, phispydir, "contigs")):
            ...
            while posn < len(genomecontig):
                lowest, posn, ss = next_location(a, posn, b)
                contigcount += 1
                if len(ss) > 0:
